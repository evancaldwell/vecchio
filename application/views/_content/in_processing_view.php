...
// count the number of new estimates
	...
	$new = $new_estimates;
 if($this->uri->segment(3) == 'ship_updated'){
...
<table width="857" border="0" cellspacing="0" cellpadding="0">
  <tr class="top_order">
    ...
    <td width="176">Ship Date</td>
    <td width="177">Status </td>
   ...
   <?php if($no != 0){
    ...
    <td width="176"><?php if($new[$i]['shipping'][0]['ship_date'] != '0000-00-00'){
		$new_date = date('m/d/Y', strtotime($new[$i]['shipping'][0]['ship_date']));
	} else {
		$new_date = '';
	}
	echo $new_date;
	 ?> </td>
    <td width="177"><?php echo $new[$i]['status_text']; ?></td>
    <td width="185">
	<?php if($no > 1){ ?>
	<?php echo anchor('vecchio_admin/inprocessing/' .$new[$i]['id'], 'View / Edit' ); ?>
	<?php } else {
	echo anchor('vecchio_admin/inprocessing/', 'Back to List' );
	} ?>
	</td>
  </tr>
   <tr>
	...
	<?php } ?>
      <td colspan="2" valign="top" >
		...
		<table>
		    ...
		    <h3 style="margin-left:0px;">Update or Choose Ship Date</h3>
			<?php echo form_open('vecchio_admin/update_ship_date/order');
			if($new[$i]['shipping'][0]['ship_date'] != '0000-00-00'){
				$new_date = date('m/d/Y', strtotime($new[$i]['shipping'][0]['ship_date']));
			} else {
				$new_date = '';
			}
			...
			?>
			<input type="text" name="ship_date" value="<?php echo $new_date ?>" id="datepickership-<?php echo $new[$i]['id'] ?>" />
      ...
      <td colspan="3" valign="top" >
		...
		$(document).ready(function() {
			$("#toship-<?php echo $new[$i]['id'];?>").validate();
		});
		</script>
      ...
      <?php 
        ...
        <div id="order_form">
	     ...
	     <h3 style="margin-left:0px;">Enter Shipped Date, Mark Order as "Shipped"</h3>
		<script> 
		$(function() {
			$( "#datepicker-<?php echo $new[$i]['id']  ?>" ).datepicker();
			$( "#datepickership-<?php echo $new[$i]['id']  ?>" ).datepicker();
		  ...
		  <?php
			...
			echo form_open('vecchio_admin/toshipped', $data);
			$data = array(
			       'id' => 'datepicker-'. $new[$i]['id'],
			 	   'name' => 'shipped_date',
				   'class' => 'required'
			);			
			echo form_input($data);
			echo form_hidden('order_id', $new[$i]['id']);
			echo form_hidden('grand_total', $new[$i]['freight']['grand_total']);
			echo form_submit('submit', 'Enter Shipped Date - Send To Shipped');
			echo form_close();
		  ?>
	  ...
	  <?php
		...
		<tr>
			<td colspan="5"><strong>No Orders In Processing </strong></td>
		</tr>
	   <?php } ?>
</table>
<br />
