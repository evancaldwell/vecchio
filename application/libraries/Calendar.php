	...
	//--------------------------------------------------------------------------------------------
		...
		function calendar_head($content='') {
			...
			if(!in_array($this->displaytype,array("list","mini-list"))) { // mini and full cal
				...
				$html .= '	
						<span>VECCHIO TREES SHIPPING CALENDAR</span></div>
					</td>';
		...
		function cal_viewmode() {
			...
			<div class="view-buttons">
				...
				<a href="'.$this->base_url.'index.php/vecchio_admin/" class="calendar-view">Back To Vecchio Admin</a>
			</div>';
			return $html;
		}
		...
		function calendar_cell($day, $class, $date = '', $style = '') {
			...
			if(count($cellevents) > 0) {
				...
				foreach($cellevents as $cellevent) {
					...
					if($cellevent['location']!='') $eventcontent .= '<div><span class="event-metalabel">Ship To:</span> <span style="white-space:nowrap">'.$cellevent['location'].'</style></div>';
					if($cellevent['link']!='') $eventcontent .= '<div><span class="event-metalabel">Order Details:</span> <a href="'.$cellevent['link'].'">'.$cellevent['linktext'].'</a></div>';
					if($cellevent['details']!='') $eventcontent .= '<div><span class="event-metalabel">Shipping Details:</span><br/>'.$cellevent['details'].'</div>';
					$eventcontent .= '</div>';
					$eventcontent .= '</li>';
					}
		...
		function calendar_foot($content='') {
			...
			$html .= '<div><table><tr><td><div style="width:10px; height:10px; background-color:#9BFFAE;">&nbsp;</div></td><td><span style="font-size:10px;"><b>= Inventory Shipping Backlog / Rep Quote</b></span></td></tr><tr><td><div style="width:10px; height:10px; background-color:#FF9876;">&nbsp;</div></td><td><span style="font-size:10px;"><b> = Shipped</b></td></tr></table></div>';
			return $html;	
		}
		...
		function addEvent($arr) {
			...
			$linktext = array_key_exists('linktext',$arr)?$arr['linktext']:''; // TYLER
			...
			if(!in_array($this->displaytype,array("list","mini-list"))) { // mini and full cal
				...
				for( $i=0; $i <= $eventdays; $i++ ) { 
					...
					if($from >= strtotime($this->year.'-'.$this->month) || strtotime($to) >= strtotime($this->year.'-'.$this->month)) {
						$this->events[] = array(
							'color'=>$color, 
							'title'=>$title, 
							'link'=>$link,
				...
				if($from >= strtotime($this->year.'-'.$this->month.'-'.$this->day) || strtotime($to) >= strtotime($this->year.'-'.$this->month.'-'.$this->day)) {
			...
			if(!in_array($this->displaytype,array("list","mini-list"))) { // mini and full cal
				...
				foreach($this->events as $event) {
					...
					if($this->displaytype=='list') { // full event list
						...
						if($event['link']!='') $html.= '<tr><td><div class="event-metalabel">Link:</div></td><td><a href="'.$event['link'].'">'.$event['linktext'].'</a></td></tr>';
						$html.= '</table></div>';	
